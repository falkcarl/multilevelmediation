...
stack_bpg <- function(data, L2ID, X, Y, M,
  ...
  if (!(L2ID %in% colnames(data))) {stop("L2ID variable not found in dataset. Please check specified name.")}
  if (!(X %in% colnames(data))) {stop("X variable not found in dataset. Please check specified name.")}
  if (!(Y %in% colnames(data))) {stop("Y variable not found in dataset. Please check specified name.")}
  if (!(M %in% colnames(data))) {stop("M variable not found in dataset. Please check specified name.")}
  ...
  if (!is.null(moderator)) {
    ...
    if (length(moderator) > 1) {stop("Only 1 moderator variable is allowed.")}
    ...
    if (moderator %in% c(X, Y, M, covars.m, covars.y, L2ID)) {stop("Moderator has same name as X, Y, M, covars.m, or covars.y, or L2ID variables.")}
  ...
  if (!is.null(covars.m)) {
    ...
    if (!all(covars.m %in% colnames(data))) {stop("One or more covariates not found in dataset. Please check specified name(s).")}
    if (any(!sapply(data[covars.m], is.numeric))) {stop("One or more covariates is not numeric. Currently, only numeric covariates are supported.")}
    if (any(covars.m %in% c(X, Y, M, moderator, L2ID))) {stop("Covariate(s) have the same name as X, Y, M, moderator, or L2ID variables.")}
  ...
  if (!is.null(covars.y)) {
    ...
    if (!all(covars.y %in% colnames(data))) {stop("One or more covariates not found in dataset. Please check specified name(s).")}
    if (any(!sapply(data[covars.y], is.numeric))) {stop("One or more covariates is not numeric. Currently, only numeric covariates are supported.")}
    if (any(covars.y %in% c(X, Y, M, moderator, L2ID))) {stop("Covariate(s) have the same name as X, Y, M, moderator, or L2ID variables.")}
  ...
  tmp <- tidyr::pivot_longer(tmp, cols = c(Y, M), names_to = "Outcome",
                      values_to = "Z")
